...
Future<void> _pickFiles(BuildContext context, Ref ref) async {
  // Align Android file picking flow with folder picking flow while still supporting file selection.
  ...
  // show loading dialog (non-dismissible) and wait a bit
  ...
  try {
    if (defaultTargetPlatform == TargetPlatform.android && (ref.read(deviceInfoProvider).androidSdkInt ?? 0) >= android_channel.contentUriMinSdk) {
      ...
      // Non-Android or older Android: fallback to file_selector/openFiles()
